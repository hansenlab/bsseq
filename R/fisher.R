fisherTests <- function(BSseq, group1, group2, lookup = NULL,
    ...
    ptime1 <- proc.time()
    M1 <- rowSums(getCoverage(BSseq, type = "M"), cols = group1)
    M2 <- rowSums(getCoverage(BSseq, type = "M"), cols = group2)
    Cov1 <- rowSums(getCoverage(BSseq, type = "Cov"), cols = group1)
    Cov2 <- rowSums(getCoverage(BSseq, type = "Cov"), cols = group2)
